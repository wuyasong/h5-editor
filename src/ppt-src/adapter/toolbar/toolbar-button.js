...
buttonComponent.prototype = {
    ...
    build: function (data) {
        var _this = this;
        // 主编辑区创建文本元素视图 并更新模型
        this.elemId = mainEdit.createElement(data, 'button');
        // 缩略图区创建文本元素视图
        thumbList.createElement(this.elemId, 'button');
        // 更新当前状态
        component.updateEditState(this.elemId, this.type);
        // 重置文本编辑及拖拽状态
        component.updateDragState();
        // 更新表单视图
        component.panel.updateButtonView();
        // 形状元素绑定右键菜单指令
        this.bindEvents();
        // 实例化缩放事件
        component.dragEvent(this.elemId, 'button');
        // 实例化拖拽事件
        component.resizeEvent(this.elemId, 'button');
        // 更新图层列表视图
        layerlist.render();
