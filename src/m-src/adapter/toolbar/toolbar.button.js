...
var domView = require('../../view/preview/buttonView.hbs');
var buttonFormCtrls = require('../form/buttonCtrls');
...
buttonAdapter.prototype = {
    init: function(){
        var _this = this;
        // 按钮元素监听点击事件
        $('.main-toolbar-button-icon').unbind('click').bind('click', function(e){
            e.stopPropagation();
            // 初始化按钮元素
            _this.getButton();
            var element = {};
            ...
            vm.execute({
                ...
                execute: _this.getButton.bind(_this, element), 
                title: '新建按钮元素', 
            });
        });
        // 实例化表单项change操作
        this.formCtrls = new buttonFormCtrls();
        vm.updateFormCtrls.button = this.formCtrls.updateContainerButton;
    },
    getButton: function(data){
        var _this = this;
        // 创建按钮唯一模型
        this.createButtonModel(data);
        // 创建按钮dom元素
        ...
        // 监听右键菜单事件
        this.elem.unbind('contextmenu').bind('contextmenu', function(e){ e.stopPropagation(); _this.buttonContextMenu(e, this); return false; });
        // 更新按钮所有表单视图
        _this.formCtrls.updateFormView();
        // 创建图层
        layerComponent.insert('按钮', store.elements[this.elemId], this.updateCurrTools);
    },
    // 创建按钮唯一模型
    createButtonModel: function(data){
        if (data) {
            // 获取id
            this.elemId = data.id;
            // 按钮模型指向缓存的数据对象
            ...
            // 获取id
            this.elemId = 'med_button_' + (+new Date());
            // 创建模型
            ...
            // 深度拷贝默认文本模型
            $.extend(true, store.elements[this.elemId], defaultStore.button);
        }
    },
    // 创建按钮dom元素
    createButtonDom: function(){
        var _this = this;
        if (!$('#' + this.elemId).length) {
            var buttonElem, scrollTop = $('.main-preview-box').scrollTop();
            // 预览视口滚动过
            if (scrollTop != 0) {
                store.elements[this.elemId].style.top = scrollTop;
            }
            buttonElem = domView(store.elements[this.elemId]);
            // 预览窗口创建dom元素生成唯一id  添加历史记录
            $('.main-preview').append(buttonElem);
        }
        // 更新当前对象属性;
    ...
    initDragEvent: function(){
        ...
        this.draggleable = new uiDraggable(this.elem, {
            ...
            onDragStart: function(draggable, elem){
                ...
                // 更新表单视图
                $('.set-button-left').val(parseInt(draggable.offsetX));
                $('.set-button-top').val(parseInt(draggable.offsetY));
                $('.set-button-width').val(parseInt(draggable.width));
                $('.set-button-height').val(parseInt(draggable.height));
                // 更新容器所有表单视图
                _this.formCtrls.updateFormView();
                // 隐藏右键菜单
                contextmenu.hide();
    ...
    initResizeEvent: function(){
        ...
        this.resizable = new uiResizable({
            ...
            onResizeStart: function(resizable, elem){
                ...
                // 更新表单视图
                $('.set-button-left').val(parseInt(resizable.left));
                $('.set-button-top').val(parseInt(resizable.top));
                $('.set-button-width').val(parseInt(resizable.width));
                $('.set-button-height').val(parseInt(resizable.height));
                // 更新容器所有表单视图
                _this.formCtrls.updateFormView();
            },
            ...
            onResizeEnd: function(resizable){
                ...
                // 更新行高
                $('#' + store.currElemId).children('.drag-ele').css('line-height', resizable.height + 'px');
                // 更新模型
                ...
                store.elements[store.currElemId].style.lineHeight = resizable.height;
            }
        });
    },
    // 更新元素id，编辑工具
    ...
    deleteHandler: function(id){
        ...
        // 添加历史记录
        vm.execute({
            undo: _this.getButton.bind(_this, element),
    ...
    // 右键菜单事件
    buttonContextMenu: function(e, self){
        var _this = this;
